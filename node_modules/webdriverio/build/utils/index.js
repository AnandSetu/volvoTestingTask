 ...
 */
export async function isWithinElementScope(node, host, handle) {
    ...
    const [documentFound, ...shadowFinds] = await Promise.all([
        browser.execute(isDescendent, host, node).then((wasFound) => wasFound ? host : undefined),
        ...shadowRoots.map((shadowRootId) => (browser.execute(isDescendent, { [ELEMENT_KEY]: shadowRootId }, node).then((wasFound) => wasFound ? shadowRootId : undefined)))
    ]);
    if (documentFound) {
        return true;
    }
    const allShadowFinds = shadowFinds.filter(Boolean);
...
export async function findDeepElement(selector) {
    ...
    const deepElementResult = await Promise.all([
         ...
         */
        browser.findElement(using, value).then(elementPromiseHandler(handle, shadowRootManager, undefined), elementPromiseHandler(handle, shadowRootManager, undefined)),
        /**
         * lookup in shadow roots
         */
        ...shadowRoots.map((shadowRootNodeId) => browser.findElementFromShadowRoot(shadowRootNodeId, using, value)
            .then(elementPromiseHandler(handle, shadowRootManager, shadowRootNodeId), elementPromiseHandler(handle, shadowRootManager, shadowRootNodeId))
            .then(async (res) => {
            if (!res) {
                return;
            }
            ...
            if ('elementId' in this) {
                return (await isWithinElementScope(res, this, handle))
                    ? res
                    : undefined;
            }
            return res;
...
export async function findDeepElements(selector) {
    ...
    const deepElementsResult = await Promise.all([
         ...
         * lookup in shadow roots
         */
        ...shadowRoots.map((shadowRootNodeId) => browser.findElementsFromShadowRoot(shadowRootNodeId, using, value)
            .then(elementPromiseHandler(handle, shadowRootManager, shadowRootNodeId), elementPromiseHandler(handle, shadowRootManager, shadowRootNodeId)).then(async (res) => {
            if (!res) {
                return;
            }
            if ('elementId' in this) {
                const allElementsWithinScope = await Promise.all(res.map((el) => isWithinElementScope(el, this, handle)
                    .then((isWithinScope) => isWithinScope ? el : undefined)));
                return allElementsWithinScope.filter(Boolean);
            }
            return res;
        }))
    ]);
     ...
     */
    return deepElementsResult.flat().filter(Boolean);
}
/**
